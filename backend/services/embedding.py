...
# Cohere配置常量
EMBEDDING_DIMENSION = int(os.environ.get("COHERE_EMBEDDING_DIMENSION", 1024))  # Cohere embedding的默认维度
EMBEDDING_BATCH_SIZE = int(os.environ.get("COHERE_EMBEDDING_BATCH_SIZE", 5))  # 每批处理的文本数量
EMBEDDING_REQUEST_TIMEOUT = int(os.environ.get("COHERE_REQUEST_TIMEOUT", 60))  # 请求超时时间(秒)
EMBEDDING_MAX_RETRIES = int(os.environ.get("COHERE_MAX_RETRIES", 3))  # 最大重试次数

print(f"[服务初始化] 使用Cohere Embedding API: {COHERE_EMBEDDING_MODEL}, 维度: {EMBEDDING_DIMENSION}")
...
class CohereEmbeddingSingleton:
    ...
    def _init_client_if_needed(self):
        ...
        if self._http_client is None:
            try:
                # 初始化HTTP客户端
                print(f"[CohereEmbeddingSingleton] 初始化Cohere HTTP客户端")
                ...
                print(f"[CohereEmbeddingSingleton] HTTP客户端初始化完成")
    ...
    def generate_batch_embeddings(self, texts: List[str]) -> List[List[float]]:
        ...
        if not COHERE_API_KEY:
            print("[CohereEmbeddingSingleton] 错误: 未设置COHERE_API_KEY，无法生成嵌入向量")
            # 返回零向量作为占位符
        ...
        try:
            ...
            if len(embeddings) != len(texts):
                print(f"[CohereEmbeddingSingleton] 警告: 返回的嵌入向量数量({len(embeddings)})与请求文本数量({len(texts)})不符")
...
def generate_embeddings(texts: List[str]) -> List[List[float]]:
    ...
    print(f"[embedding] 使用Cohere模型({COHERE_EMBEDDING_MODEL})为{len(texts)}个文本生成嵌入向量")
    ...
    for i in range(0, len(texts), batch_size):
        ...
        for retry in range(1, max_retries + 1):
            try:
                batch_embeddings = get_embedding_model().generate_batch_embeddings(batch)
                all_embeddings.extend(batch_embeddings)
                print(f"[embedding] 批次 {batch_num} 处理成功")
                break
            except Exception as e:
                print(f"[embedding] 批次 {batch_num} 处理失败 (尝试 {retry}/{max_retries}): {str(e)}")
                ...
                if retry < max_retries:
                    ...
                    print(f"[embedding] 批次 {batch_num} 达到最大重试次数，使用零向量替代")
                    # 对于失败的批次，使用零向量替代
                    zero_embeddings = [[0.0] * EMBEDDING_DIMENSION for _ in range(len(batch))]
